...
from .forms import ReportProductForm
from .models import ProductReport

@login_required
def report_product_view(request, pk):
    product = get_object_or_404(Product, pk=pk)
    if request.method == 'POST':
        form = ReportProductForm(request.POST)
        if form.is_valid():
    ...
    return render(request, 'products/report_product.html', {'form': form, 'product': product})
...
def get_average_rating(product):
    return ProductReview.objects.filter(product=product).aggregate(avg=Avg('rating'))['avg'] or 0
